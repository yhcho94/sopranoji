...
export default function PastPerformances({ items }: { items: Performance[] }) {
  ...
  const years = items.map((item) => Number(getYear(item.date)));
  const minYear = Math.min(...years);
  const maxYear = Math.max(...years);
  const rangeLabel =
    minYear === maxYear ? `${minYear}년` : `${minYear}~${maxYear}년`;
  ...
  return (
    <div className="mt-10">
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-line bg-background-elevated">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
        ...
        >
          <span className="text-sm text-foreground sm:text-base">
            {rangeLabel} <span className="text-muted">({items.length}건)</span>
          </span>
          <svg
            viewBox="0 0 24 24"
            fill="none"
